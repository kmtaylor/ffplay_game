...
int video_open(VideoState *is, int force_set_video_mode, VideoPicture *vp)
{
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts, int64_t pos, int serial)
{
    ...
    if (vp->bmp) {
        ...
        sws_scale(is->img_convert_ctx, (const uint8_t **) 
...
void ffplay_event_loop(VideoState *cur_stream)
{
    SDL_Event event;
...
VideoState *ffplay_init(const char *resource_file)
{
    int flags;
    VideoState *is;
    char dummy_videodriver[] = "SDL_VIDEODRIVER=dummy";
    
    input_filename = resource_file;
    ...
    flush_pkt.data = (unsigned char *)(intptr_t)"FLUSH";
